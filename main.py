...
# Auto-find cookies (WZML-X style)
COOKIES_FILE = None
...
#             SESSION STORE
# ═══════════════════════════════════════════
URL_SESSIONS = {}   # uid → session
PL_SESSIONS  = {}   # uid → playlist session
WAITING_SEL  = {}   # user_id → pl_uid
...
def safe_name(s, n=180): return re.sub(r'[^\w\s\-\.]', '', s or "file")[:n].strip() or "file"
...
def is_playlist_url(url):
    return bool(re.search(r"(playlist\?list=|/playlist/)", url, re.I))
...
# ═══════════════════════════════════════════
#    yt-dlp OPTIONS (WZML-X exact + Deno Fix)
# ═══════════════════════════════════════════
def _base_opts():
    o = {
        "usenetrc": True,
        ...
        "remote_components": ["ejs:github"],  # DENO BYPASS ADDED HERE
    }
...
def _info_opts():
    ...
    o.update({
        ...
        "no_warnings": True, 
        "playlist_items": "0",
        "format": "bv*+ba/b"  # PREVENTS FORMAT NOT AVAILABLE CRASH
    })
    return o
...
def _dl_opts(fmt, out_tmpl, tracker=None, is_pl=False):
    ...
    # Thumbnail embed for both audio and video
...
# ═══════════════════════════════════════════
#    FORMAT PARSER (WZML-X exact logic)
# ═══════════════════════════════════════════
...
def _blocking_playlist_info(url):
    opts = _info_opts()
    opts.update({"extract_flat": True, "playlistend": MAX_PLAYLIST})
    try:
    ...
    if uid in WAITING_SEL:
        ...
        if e:
            ...
            try:
                ...
                msg_t, kb = _kb_main(fmts, s_uid, True, 120) # TRUE KAR DIYA (Playlist Fix)
...
async def quality_cb(client, query: CallbackQuery):
    ...
    if action == "fmt":
        ...
        # PLAYLIST SHORTCODE PARSER FIX
        if "|" in qual and len(qual.split("|")) == 2:
            h, ext = qual.split("|")
            if ext == "mp4":
                qual = f"bv*[height<=?{h}][ext=mp4]+ba[ext=m4a]/b[height<=?{h}]/bv*+ba/b"
            else:
                qual = f"bv*[height<=?{h}][ext=webm]+ba/b[height<=?{h}]/bv*+ba/b"
        elif qual in fmts:
            qual = fmts[qual]
...
async def _dl_single(url, qual, info, smsg, client, chat_id):
    loop     = asyncio.get_event_loop()
    uid      = _new_uid()
    title_s  = safe_name(info.get("title","video"), 150)
    dl_dir   = os.path.join(DOWNLOADS_DIR, uid)
    os.makedirs(dl_dir, exist_ok=True)
    out_tmpl = os.path.join(dl_dir, f"{title_s}.%(ext)s")
...
async def _dl_playlist(entries, qual, base_info, smsg, client, chat_id, pl_uid):
    ...
    for idx, en in enumerate(entries, 1):
        ...
        uid     = _new_uid()
        title_s = safe_name(vi.get("title", vtitle), 150)
        dl_dir  = os.path.join(DOWNLOADS_DIR, uid)
        os.makedirs(dl_dir, exist_ok=True)
        out_tmpl = os.path.join(dl_dir, f"{title_s}.%(ext)s")
