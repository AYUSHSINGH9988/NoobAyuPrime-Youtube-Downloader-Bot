...
def _blocking_download(url, fmt, out_tmpl, smsg, loop, is_pl=False):
    try:
        ...
        with yt_dlp.YoutubeDL(opts) as ydl:
            ...
            # BUG FIX: prepare_filename might not match actual file with new template
            # Scan directory for the real downloaded file
...
async def upload_file(client, chat_id, result, fmt, smsg):
    ...
    # WZML-X style: include resolution, fps, tbr in caption
    height = info.get("height") or 0
    fps    = info.get("fps") or 0
    tbr    = int(info.get("tbr") or 0)
    ext    = os.path.splitext(fp)[-1].lstrip(".")

    # Build quality string like "1080p25fps 3033kbps"
    quality_str = ""
    if height:   quality_str += f"{height}p"
    if fps:      quality_str += f"{int(fps)}fps"
    if tbr:      quality_str += f" {tbr}kbps"

    cap = (
        f"🎬 **{title}**\n"
        f"👤 `{upl}`\n"
        f"⏱️ `{time_fmt(dur)}`  📦 `{humanbytes(sz)}`"
        + (f"\n🎞️ `{quality_str} {ext}`" if quality_str else "")
    )
    ...
    try:
        ...
        if is_audio:
            await client.send_audio(chat_id, fp, caption=cap, duration=dur,
                title=title[:64], performer=upl, thumb=thumb,
...
async def show_quality_picker(url, smsg, user_id=None):
    ...
    try:
        if thumb: await smsg.reply_photo(photo=thumb, caption=info_txt+q_msg, reply_markup=kb)
        else: raise Exception()
    except Exception:
        with suppress(Exception): await smsg.reply_text(info_txt+q_msg, reply_markup=kb)
...
async def _dl_single(url, qual, info, smsg, client, chat_id):
    ...
    # WZML-X style filename: "Title height p fps fps tbr .ext"
    # %(height)s, %(fps)s, %(tbr)d auto-filled by yt-dlp
    out_tmpl = os.path.join(dl_dir,
        "%(title,fulltitle,alt_title)s%(height& |)s%(height|)s%(height&p|)s%(fps|)s%(fps&fps|)s%(tbr& |)s%(tbr|)d.%(ext)s"
    )
...
async def _dl_playlist(entries, qual, base_info, smsg, client, chat_id, pl_uid):
    ...
    for idx, en in enumerate(entries, 1):
        # BUG FIX: extract_flat entries mein "url" ya "webpage_url" nahi hoti
        # YouTube entries mein sirf "id" hota hai — full URL reconstruct karo
        vurl = (
            en.get("url") or
            en.get("webpage_url") or
            (f"https://www.youtube.com/watch?v={en['id']}" if en.get("id") else None)
        )
        if not vurl:
        ...
        await _safe_edit(smsg, f"{prefix}🔍 Info...")
        try:
            vi = await loop.run_in_executor(None, _blocking_info, vurl)
            if not vi: raise ValueError()
        except Exception:
            failed.append(vtitle); continue
        uid     = _new_uid()
        dl_dir  = os.path.join(DOWNLOADS_DIR, uid)
        os.makedirs(dl_dir, exist_ok=True)
        # WZML-X style filename with resolution/fps/tbr
        out_tmpl = os.path.join(dl_dir,
            "%(title,fulltitle,alt_title)s%(height& |)s%(height|)s%(height&p|)s%(fps|)s%(fps&fps|)s%(tbr& |)s%(tbr|)d.%(ext)s"
        )
